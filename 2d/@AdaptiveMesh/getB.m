function [B, gamma] = getB(obj, cache)
    ...
    for i = 1:nrigid
        ...
        vals( nextInd:nextInd+count-1 ) = - (obj.p(ind) - body.Position(2)); 
        nextInd = nextInd + count;
        ii( nextInd:nextInd+count-1 ) = ind;
        jj( nextInd:nextInd+count-1 ) = nelastic + i*3;
        vals( nextInd:nextInd+count-1 ) = obj.p(ind-1) - body.Position(1); 
        nextInd = nextInd + count;        
    end
    
    gamma = sparse( ii, jj, vals, numel(obj.p), numel(obj.ElasticDOFs) + 3*nrigid);
