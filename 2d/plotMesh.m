function singleUseFigures = plotMesh( h, mesh2d, settings, singleUseFigures, cache, integrator, rigidificator)
    ...
    edgeCol = iif(settings.DrawEdges, settings.elementLineColor, 'none');
...
%     tintCol = [0.8 0.5 0.2];  % orange
    ...
    if isa(mesh2d, 'AdaptiveMesh')
        if settings.plotLayers && numel(cache.layers) > 0
            sortedLayers = unique(integrator.layers);
            totalLayers = numel(sortedLayers);
            for layer = totalLayers:-1:1
                layerPercent = sortedLayers(layer);
                if layerPercent == 0
                    continue;
                end
                inds = cache.layers(:,layer)~=0;
                proportion = (layer-2)/totalLayers;
                color = (1-proportion)*[1, 0.49, 0.0]+proportion*[0.3,0.46,0.8];
                faceColor(inds, :) = repmat(color,sum(inds),1);
            end
        elseif settings.DrawGraphColors
            faceColor = mesh2d.elementRGB(mesh2d.elementColor,:);
        elseif settings.plotRigidification
    ...
    if mesh2d.RenderPatch == 0
        mesh2d.RenderPatch = patch('vertices', pr, 'faces', mesh2d.t, 'edgecol', edgeCol, 'facecol', 'flat', 'FaceVertexCData', faceColor, 'FaceAlpha', settings.elementFaceAlpha, 'EdgeAlpha', settings.elementLineAlpha, 'LineWidth', settings.elementLineThickness);    
    else
    ...
    if ( numel(mesh2d.BoundaryLines) == 0 )
        ...
        for j=1:numel(mesh2d.boundaryEdges)
            ...
            mesh2d.BoundaryLines{j} = plot( boundaryx, boundaryy, settings.boundaryColor, 'LineWidth',settings.boundaryThickness);
            axis off;
        end
    else
        for j=1:numel(mesh2d.boundaryEdges)
    ...
    if isa(mesh2d, 'AdaptiveMesh')
        ...
        if settings.DrawEDots && ~isempty(mesh2d.RigidificationValues)
            F = cache.ActiveB * (mesh2d.p + mesh2d.prevp)/2;
            Fdot = cache.ActiveB * mesh2d.v;
            ...
            croppedEdots = log10(EDotNorms);
            cut = 6;
            croppedEdots(croppedEdots < -cut) = -cut;
            croppedEdots(croppedEdots > cut) = cut;
            color = normalize(croppedEdots, 'range', [0, 1]);
            f = scatter(center(1,:), center(2,:), 5, [color,1-color,zeros(size(color))], 'filled');
            singleUseFigures = [singleUseFigures,f];
