...
mesh2d.setRigidTransform(90,[5,15],true);
...
[R, COM, angle] = RigidBody.rigidSymplecticImperativeStep(COM, angle, h, COMDOT, angularV_out);

%update pos wrt stepped rigids
xi = rotateAllVerts2D(ones(mesh2da.N,1), ri, R, COM, mesh2da.p);

steppedVelocityR = cat(3,R); 
rigidResidualv= BDF.BDF1(xi, mesh2da.p,h);

normOfRotatedVelocitiesDiff = norm(mesh2da.v - rigidResidualv);
disp("norm of all resv test diff:"+string(normOfRotatedVelocitiesDiff));
