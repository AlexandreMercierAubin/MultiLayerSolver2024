...
splitITs = floor(iterations/3);
rigidIT = [splitITs,splitITs];
...
settings.SceneName = 'cantilever_xpbdLayerNumberIterations';
...
settings.residualComparisonLabels = ["split","", "doubling","", "halving",""];
settings.specialResidualOption = 2;
settings.campos = [5,0];
settings.CamPadding = [10,10,10,10];
...
td = simulate( {mesh2da2,mesh2da3,mesh2da4}, {integrator2, integrator3,integrator4}, h, settings, rigid, NullContactFinder(), NullContactFinder(), NullAnimationScript(), StVenantKirchoffEnergy());
