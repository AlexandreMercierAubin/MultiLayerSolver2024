...
iterations = 15; %using 200 when testing features
...
integrator = xpbdLayer2D();%elastic
...
layers = [0,25,50,75,100,75,50,25,0];
integrator3.setLayers(layers);
rigidIT = [1,1,1,1,1,1,1,1];
...
settings.SceneName = 'cantilever_xpbdLayerCycles';
...
settings.residualComparisonLabels = ["linear layers", "halving layers"];
settings.plotList = [0,1,1];

td = simulate( {mesh2da,mesh2da2,mesh2da3,mesh2da4}, {integrator, integrator2,integrator4}, h, settings, rigid, NullContactFinder(), NullContactFinder(), NullAnimationScript(), StVenantKirchoffEnergy());
