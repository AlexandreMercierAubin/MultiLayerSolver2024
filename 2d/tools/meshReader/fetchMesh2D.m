function mesh2d = fetchMesh2D(fileName, resetMesh, mergeFunction, materials, settings,h)
    ...
    if isfile([tmpPath2,'.mat']) && ~resetMesh
        ...
        %TODO: set merge functs as objects with generated ids... or maybe
        %just md5 the function call see if it is the same?
        
        if isequal(mesh2d.cacheMergeMaterials, materials) && all(md5Res == mesh2d.cacheMD5)
            return;
        end
    end
    mesh2d = mergeFunction(materials,h);
