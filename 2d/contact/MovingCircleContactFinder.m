...
%             obj.dcdt = @(t) [ 0, 0.3*2*sin(t*6)*cos(t*6)*6 ];
        ...
        function [J, phi, cInfo] = findContacts( obj, meshes, time )
            ...
            for i = 1:numel(phi) 
                cInfo(i) = contactInfo( [xs(indices(i)), ys(indices(i))], normal(i,:), tangent(i,:), obj.FrictionCoefficient, indices(i), obj.ID, phi(i));
        ...
        function render( obj, time )
            ...
            if ( obj.plotHandle == 0 )
                hold on;        
                obj.plotHandle = plot( xdata, ydata,  'Color',obj.color,'LineWidth', 0.5 );
            else
