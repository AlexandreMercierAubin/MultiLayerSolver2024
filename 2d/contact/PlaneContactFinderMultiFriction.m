    ...
    methods
        ...
        function [J, phi, cInfo] = findContacts( obj, meshes, ~ )
            ...
            for i = 1:numel(phi) 
                ...
                cInfo(i) = contactInfo( [xs(indices(i)), ys(indices(i))], normal(i,:), tangent(i,:),mu, indices(i), obj.ID, phi(i));
            end     
...
%             J = J(any(J, 2), :);
%             
%             % TODO: a single node could come into contact with two planes
%             % and would have a contact jacobian for each, and would want a
%             % warm start on each.  The contact info indicies should
%             % probably include other indices, such as negative numbers,
%             % which identify each stationary boundary used in collision
%             % detection.
%             
%             indices = find(idx);
%             cInfo = contactInfo.empty;
