classdef MeshSCD < ContactFinder
    ...
    methods
        ...
        function [Jc, phi, cInfo] = findContacts( obj, mesh, ~)
            ...
            Jc = sparse(Jc); % TODO: Fix so that this is sparse to begin with! 
        end
        ...
        function [ Jc, phi, cInfo ] = findMeshContacts( obj, mesh, bes1, bes2 )
            ...
            % TODO: looks dangerouns to concatenate edges... but we will
            ...
            for j=2:numel(mbes2)  
                ...
                % TODO: ARGH! NOT ROBUST... is there an alternative SDF
                % function in gptoolbox to use here?  Perhaps don't worry
                % about it!
                % ARGH... this is broken...  will ONLY work for a mesh with
                % a hole if the hole comes second!  :(
            ...
            direction = v2-v1;
            alphas = (sum((m1ContactPoints-v1).*(direction),2) ./ sum((direction).*(direction),2));
            ...
            for i = 1:numContacts
                ...
                %TODO: test with sort, see if it helps in a difficult scene
