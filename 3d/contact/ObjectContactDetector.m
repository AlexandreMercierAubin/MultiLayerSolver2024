classdef ObjectContactDetector < ContactFinder
    ...
    methods
        function obj = ObjectContactDetector(filename, degrees, scale, position, frictionCoefficient, plotRatio)
            if nargin < 6
                plotRatio = 1;
            end
            ...
            obj.V = (R*obj.V')';
            obj.V = (scale.*obj.V);
            obj.plotV = plotRatio.*obj.V;
            obj.V(:,1) = obj.V(:,1) + position(1);
            obj.V(:,2) = obj.V(:,2) + position(2);
            obj.V(:,3) = obj.V(:,3) + position(3);
            obj.plotV(:,1) = obj.plotV(:,1) + position(1);
            obj.plotV(:,2) = obj.plotV(:,2) + position(2);
            obj.plotV(:,3) = obj.plotV(:,3) + position(3);
        ...
        function render( obj, ~ )
            ...
            hold on;
            obj.plotHandle = patch('Faces',obj.F,'Vertices',obj.plotV,'FaceColor',obj.faceColor,'EdgeColor', obj.edgeColor);
        end
