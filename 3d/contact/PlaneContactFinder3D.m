classdef PlaneContactFinder3D < ContactFinder
    ...
    methods
        ...
        function [J, phi, cInfo, contactCompliance] = findContacts( obj, meshes, time, p )
            ps = vertcat(p);
            xs = ps(1:3:end);
            ys = ps(2:3:end);
            zs = ps(3:3:end);
            ...
            for i = 1:numel(phi) 
                cInfo(i) = contactInfo3D( [xs(indices(i)), ys(indices(i)), zs(indices(i))], normal(i,:), tangent(i,:), obj.FrictionCoefficient, indices(i), obj.ID, tangent2(i,:),phi(i), obj.xpbdContactCompliance);
            end
        end
        
        function render( obj, ~ )
            if ( obj.plotHandle ~= 0 || ~obj.Visible)
                return
            end
            ...
            tl = [ tl1; tl2 ];
            obj.plotHandle = plot3( tl(:,1), tl(:,2), tl(:,3), 'r', 'LineWidth', 0.1);
