function [B, gamma] = peekB(obj, cache, bodyPositions, meshp)
    ...
    for i = 1:nrigid
        ...
        p1 = -(meshp(ind1) - bodyPositions(i,1));
        p2 = -(meshp(ind2) - bodyPositions(i,2));
        p3 = -(meshp(ind3) - bodyPositions(i,3));
        %middle top p3
        ii( nextInd:nextInd+count-1 ) = ind2;
    ...
    gamma = sparse( ii, jj, vals, numel(obj.p),  numel(obj.ElasticDOFs) + 6*nrigid);
