classdef AdaptiveMesh3D < Mesh3D
    properties
        ...
        rigidIDbyVert %#vert sized vector output from mexRigidificatior that gives us the rigid body to which a vertex belongs

        AlwaysRigidElements %hack array to force some elements to always be rigid... easiest way to implement rigid bodies without changing the code
    end
     methods(Static = true)
        function [R, com, inertia, rigidMasses, ri, VertexDisp, comdot, angularVelocity]=makeRigidsFromConnectivity(numRigid, p, v, rigidVerts, isComponentPinned, rigidIDbyVert, rigidIDbyElement, ElasticDOFs, particlemasses, unpinnedDOFs)
            %USED ONLY FOR THE MULTILAYER SOLVES
            %requires mesh2d.rigidIDbyVert and rigidIDbyElement to be defined
            [com, comdot, rigidMasses, inertia, angularVelocity, VertexDisp] = RigidBody3D.matComputeRigidBodyProperties3D(numRigid, rigidIDbyVert, p, v, particlemasses);

            R = repmat(eye(3,3),[1,1,numRigid]);

            ri = zeros(numel(rigidIDbyVert),3);
            isRigidVert = rigidIDbyVert ~=0;
            pFormatted = formatPositions3D(p);
            ri(isRigidVert,:) = pFormatted(isRigidVert,:) - permute(com(:,:,rigidIDbyVert(isRigidVert)),[3,2,1]);
        end
    end
    methods
        ...
        function labelElasticTriangles( mesh )
             ...
             for i=mesh.ElasticTetInds
                 pos = sum( pt( :, mesh.t(i,:) ), 2 ) / 3;
                 text( pos(1), pos(2), pos(3), ""+i );
             end        
        end
        
        function applyAccumulator(mesh,  rigidIDs, accumulator)
            %rigidIDs a #3N vertices vector of rigid IDs per dofs
            %accumulator a #3N vertices vector of x,y,z force to apply to vertices
