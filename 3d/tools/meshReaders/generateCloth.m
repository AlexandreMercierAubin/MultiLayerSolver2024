function mesh = generateCloth(attributes, materials, precision, scale, distMeshDimensions, perturbationNormalScaling, meshingDirection)
...
%     fd = @(p) drectangle(p, -halfdim(1), halfdim(1), -halfdim(2), halfdim(2));
    ...
    rands = randn(size(p,1),1);
    pert = -perturbationNormalScaling + rands*perturbationNormalScaling*2;
    ...
    J = [1:size(T,1)]';
    mesh = Mesh3D(V,T, attributes, materials, T, J,[],[],[],[]);
%     mesh = Mesh3D(V,T, attributes, materials, T, J,eye(size(V,1)),T,V,1:size(T,1));
end