function quickSolve3D( cache, integrator, mesh3D, h, Jc, phi, settings, animationScripter, frame)
    ...
    if integrator.useFullAinv
        ...
        if settings.addBendingEnergy
            H = cache.shellBendingH(ii,ii);
            K = K + H;
        end
        A = M - h * (-Md + Kd) - h * h * K;     
        Afun = @(v) A*v;

        fext = -h*(Md*v) + h*Kd*v + h*h*K*v;
        rhs = rhs + fext;
    else
        ...
        if settings.addBendingEnergy
            H = cache.shellBendingH(ii,ii);
            Afun = @(v) qspcgHelper3D( h, M, Md, bigB, bigAlpha1, bigC, v) + h*h*H*v; %H is the negative hessian so this should be ok
        else
...
%                Jcn = Jc( JcnInds, ii );
    ...
    if settings.PCGiterations == 1 %This version should be much faster for 1 iteration by avoiding wasteful computations
        ...
        cache.ApproximatedDeltaV(ii) = solvePCG( settings.PCGiterations, Afun, rhs, zeros(size(rhs)), cache.preconditioner );
    end

    %scripted animations positions
    for i=1:numel(animationScripter)
        [mesh3D.p, cache.ApproximatedDeltaV] = animationScripter{i}.scriptPositions(mesh3D.p, cache.ApproximatedDeltaV, frame, h);
    end
