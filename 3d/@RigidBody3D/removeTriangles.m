...
%     vertices = setdiff(obj.Indices, [obj.Mesh.t(tris, :)]);
    ...
    % find new tets
    dim = size(obj.Mesh.t,2);
    if numel(vertices) < dim
        tris = [];
    else
        % the element index must be found 4 times, once for each adjacent
        % vertex, if it is to remain part of a rigid body
        
        tris = find(accumarray([obj.Mesh.TetsPerParticle{vertices}]', 1) == dim)';
    end
    ...
    tooSmall = cellfun(@(x) numel(x) < dim, vertices);  % TODO: can this ever happen?  How?
    vertices(tooSmall) = [];
