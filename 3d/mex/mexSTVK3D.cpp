...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    for ( int el = 0; el < m; el++ ) {
        ...
        psi[el] = volume*(mu*(pow(t2+t5+t8-1.0,2.0)/4.0+pow(t3+t6+t9-1.0,2.0)/4.0+pow(t4+t7+t10-1.0,2.0)/4.0+pow(t11+t18+t26,2.0)/2.0+pow(t12+t20+t27,2.0)/2.0+pow(t13+t21+t28,2.0)/2.0)+(lambda*pow(t2+t3+t4+t5+t6+t7+t8+t9+t10-3.0,2.0))/8.0);
    }
