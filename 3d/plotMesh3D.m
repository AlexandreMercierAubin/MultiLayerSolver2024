    ...
    if settings.DrawContact
        ...
        for i = 1 : numel(cInfos)
            ...
            if numel(mesh3D.DOFsSpheres) < mesh3D.N
                ...
                set(mesh3D.DOFsSpheres(i),'FaceColor',[0 0 0], 'FaceAlpha', 0.5);
            end
        end
        
    elseif settings.DrawRigidDOFs
        faceAlpha = 0.5;
...
if ~isempty(mesh3D.SurfaceWeights)
    ...
    if isempty(mesh3D.SurfaceColors)
        surfaceFaceColor = faceColor(mesh3D.Surface2Tet,:);
    else
        surfaceFaceColor = mesh3D.SurfaceColors;
    end

    if mesh3D.RenderPatch == 0
        mesh3D.RenderPatch = patch('vertices', nV, 'faces', mesh3D.SurfaceFaces,'facecol', 'flat', 'FaceVertexCData', surfaceFaceColor , 'FaceAlpha', faceAlpha, 'EdgeColor',settings.elementLineColor, 'LineWidth', settings.elementLineThickness); 
    else
    ...
    if mesh3D.RenderPatch == 0
        if plotFacesOnly
            mesh3D.RenderPatch = patch('vertices', pr, 'faces', mesh3D.faces, 'facecol', 'flat', 'FaceVertexCData', faceColor , 'FaceAlpha', faceAlpha, 'EdgeColor',settings.elementLineColor, 'LineWidth', settings.elementLineThickness); 
        else
            mesh3D.RenderPatch = patch('vertices', pr, 'faces', mesh3D.t, 'facecol', 'flat', 'FaceVertexCData', faceColor , 'FaceAlpha', faceAlpha, 'EdgeColor', settings.elementLineColor, 'LineWidth', settings.elementLineThickness); 
        end
    else
...
elseif settings.DrawForces
    f =  cache.bendingForces;%cache.elasticForces +
    if numel(f) > 0
        for i = 1:mesh3D.N
            xdata = [mesh3D.p(i*3-2);mesh3D.p(i*3-2)+f(i*3-2)];
            ydata = [mesh3D.p(i*3-1);mesh3D.p(i*3-1)+f(i*3-1)];
            zdata = [mesh3D.p(i*3);mesh3D.p(i*3)+f(i*3)];
            if numel(mesh3D.lineHandles) < i
                mesh3D.lineHandles = [mesh3D.lineHandles;line(xdata,ydata,zdata, 'Color',[100/255,100/255,255/255])];
            else
                mesh3D.lineHandles(i).XData = xdata;
