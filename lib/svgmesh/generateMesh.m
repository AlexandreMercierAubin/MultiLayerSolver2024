function mesh = generateMesh( filename, varargin )
    ...
    %
    % TODO: some svg files have grouping which will cause the svg loader to
    % fail... could be made more robust.  
    ...
    if triangulate
        if (useProvidedPoly == true ) 
            system( pwd + "/lib/svgmesh/triangle.exe -p -q30 -D -A -a" + maxArea  + " " + filename );
            ...
            system( pwd + "/lib/svgmesh/triangle.exe  -p -q" + minimumAngle + "-D -A -a" + maxArea  + " " + filepath + "/" + modelname + ".poly" );
            [TV,I] = readNODE( filepath + "/" + modelname + ".1.node" );
            [TF,A] = readELE( filepath + "/" + modelname + ".1.ele");
